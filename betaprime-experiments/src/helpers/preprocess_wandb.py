...
def save_covariance_matrix(project_name, dataset_name, cov_matrix):
    # Save the covariance matrix as an artifact
    # download the training data artifact and save the covariance matrix
    run = wandb.init(project=project_name, job_type="save_covariance_matrix")
    artifact = run.use_artifact(dataset_name)
    artifact_dir = artifact.download()
    cov_train = multi_signal_cov_calculator(cov_matrix)
    upload_

def check_valid_covariance_matrix(artifact):
    # TODO: Implement the logic to check if the covariance matrix is valid 
    # by retreiving the covariance matrix from the saved artifact and checking its properties
