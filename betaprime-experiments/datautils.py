...
def single_signal_cov_calculator (signal, eeg_ch=22, eog_ch=3, remove_eog=False, type = 'SCM'):
    '''
    function for calculating covariance matrix of EEG signals
    if remove_eog is True, it will remove EOG artifact
    '''
    # print(f'Shape of signal: {signal.shape}')
    X = signal[:-eog_ch, :] if remove_eog else signal
    # Initialize covariance matrix
    cov_matrix = np.zeros((X.shape[1], X.shape[1]))
    if type == 'SCM': # Sample Covariance)
        cov_matrix = X @ X.T  # Sample covariance matrix
        cov_matrix /= X.shape[1] - 1  # Normalize by the number of samples
        if remove_eog:
            b = eog_remove(signal, eeg_ch, eog_ch)
            cov_matrix -= np.outer(b, b.T).dot(cov_matrix[-eog_ch:, :])
    
    elif type == 'numpy': # Numpy Covariance
        cov_matrix = np.cov(X, rowvar=True)
        if remove_eog:
            b = eog_remove(signal, eeg_ch, eog_ch)
            cov_matrix -= np.outer(b, b.T).dot(cov_matrix[-eog_ch:, :])
    elif type == 'oas':
        # OAS (Orthogonalized Approximate Sample Covariance)
        cov_matrix = np.cov(X, rowvar=True)
        # cov_matrix += np.eye(cov_matrix.shape[0]) * 1e-5
    
    return cov_matrix

def multi_signal_cov_calculator (signals, eeg_ch=22, eog_ch=3, remove_eog=False, type = 'SCM'):
    '''
    function for calculating covariance matrix of multiple EEG signals
    if remove_eog is True, it will remove EOG artifact
    '''
    cov_matrices = []
    for signal in signals:
        cov_matrix = single_signal_cov_calculator(signal, eeg_ch, eog_ch, remove_eog, type)
...
def mat_extractor (
    ...
    for j in range(3,N):
        ...
        for i in range(48):
            if i < 47: # if not the last trial
                x = samples[trials[i,0]:trials[i+1,0]]
            else:
                x = samples[trials[i,0]:]
            
            if remove_eog: # If we want to remove EOG artifact
